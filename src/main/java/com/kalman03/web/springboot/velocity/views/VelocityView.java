 ...
 * <ul>
 * <li><b>url</b>: the location of the Velocity template to be wrapped, relative
 * to the Velocity resource loader path (see VelocityConfigurer).
 * <li><b>encoding</b> (optional, default is determined by Velocity
 * configuration): the encoding of the Velocity template file
 ...
 * view, or {@code null} if not needed. VelocityFormatter is part of standard
 * Velocity.
 * <li><b>dateToolAttribute</b> (optional, default=null): the name of the
 * DateTool helper object to expose in the Velocity context of this view, or
 * {@code null} if not needed. DateTool is part of Velocity Tools.
 * <li><b>numberToolAttribute</b> (optional, default=null): the name of the
 * NumberTool helper object to expose in the Velocity context of this view, or
 * {@code null} if not needed. NumberTool is part of Velocity Tools.
 * <li><b>cacheTemplate</b> (optional, default=false): whether or not the
 * Velocity template should be cached. It should normally be true in production,
 * but setting this to false enables us to modify Velocity templates without
 * restarting the application (similar to JSPs). Note that this is a minor
 * optimization only, as Velocity itself caches templates in a
 * modification-aware fashion.
 * </ul>
 *
 * <p>
 ...
 *
 * <p>
 * Note: Spring 3.0's VelocityView requires Velocity 1.4 or higher, and
 * optionally Velocity Tools 1.1 or higher (depending on the use of DateTool
 * and/or NumberTool).
 *
 * @author Rod Johnson
...
public class VelocityView extends AbstractTemplateView {
	...
	/**
	 * Set tool attributes to expose to the view, as attribute name / class name
	 * pairs. An instance of the given class will be added to the Velocity context
	 * for each rendering operation, under the given attribute name.
	 * <p>
	 * For example, an instance of MathTool, which is part of the generic package of
	 * Velocity Tools, can be bound under the attribute name "math", specifying the
	 * fully qualified class name "org.apache.velocity.tools.generic.MathTool" as
	 * value.
	 * <p>
	 * Note that VelocityView can only create simple generic tools or values, that
	 * is, classes with a public default constructor and no further initialization
	 * needs. This class does not do any further checks, to not introduce a required
	 * dependency on a specific tools package.
	 * <p>
	 * For tools that are part of the view package of Velocity Tools, a special
	 * Velocity context and a special init callback are needed. Use
	 * VelocityToolboxView in such a case, or override {@code createVelocityContext}
	 * and {@code initTool} accordingly.
	 * <p>
	 * For a simple VelocityFormatter instance or special locale-aware instances of
	 * DateTool/NumberTool, which are part of the generic package of Velocity Tools,
	 ...
	 * of this view, or {@code null} if not needed. The exposed DateTool will be
	 * aware of the current locale, as determined by Spring's LocaleResolver.
	 * <p>
	 * DateTool is part of the generic package of Velocity Tools 1.0. Spring uses a
	 * special locale-aware subclass of DateTool.
	 * 
	 ...
	 * context of this view, or {@code null} if not needed. The exposed NumberTool
	 * will be aware of the current locale, as determined by Spring's
	 * LocaleResolver.
	 * <p>
	 * NumberTool is part of the generic package of Velocity Tools 1.1. Spring uses
	 * a special locale-aware subclass of NumberTool.
	 * 
	...
	/**
	 * Set the encoding of the Velocity template file. Default is determined by the
	 * VelocityEngine: "ISO-8859-1" if not specified otherwise.
	 * <p>
	...
	/**
	 * Set whether the Velocity template should be cached. Default is "false". It
	 * should normally be true in production, but setting this to false enables us
	 * to modify Velocity templates without restarting the application (similar to
	 * JSPs).
	 * <p>
	 ...
	 * <p>
	 * If this is not set, the default lookup will occur: A single VelocityConfig is
	...
	/**
	 * Autodetect a VelocityEngine via the ApplicationContext. Called if no explicit
	 * VelocityEngine has been specified.
	 * 
	 ...
	 * <p>
	 * Called by {@code renderMergedTemplateModel}. The default implementation is
	 * empty. This method can be overridden to add custom helpers to the model.
	 * 
	 * @param model   the model that will be passed to the template for merging
	 * @param request current HTTP request
	 * @throws Exception if there's a fatal error while we're adding model
	 *                   attributes
	...
	/**
	 * Create a Velocity Context instance for the given model, to be passed to the
	 * template for merging.
	 * <p>
	 * The default implementation delegates to {@link #createVelocityContext(Map)}.
	 * Can be overridden for a special context class, for example ChainedContext
	 * which is part of the view package of Velocity Tools. ChainedContext is needed
	 * for initialization of ViewTool instances.
	 * <p>
	 ...
	 * in order to avoid a required dependency on the view package of Velocity
	 * Tools.
	 * 
	 * @param model    the model Map, containing the model attributes to be exposed
	 *                 to the view
	 * @param request  current HTTP request
	 ...
	 * 
	 * @param model the model Map, containing the model attributes to be exposed to
	 *              the view
	 * @return the Velocity Context
	 * @throws Exception if there's a fatal error while creating the context
	 ...
	 * to {@code exposeHelpers(velocityContext, request)}. This method can be
	 * overridden to add special tools to the context, needing the servlet response
	 * to initialize (see Velocity Tools, for example LinkTool and
	 ...
	 * helpers to the Velocity context.
	 * 
	 * @param velocityContext Velocity context that will be passed to the template
	 * @param request         current HTTP request
	 * @throws Exception if there's a fatal error while we're adding model
	 ...
	 * properties. Override one of the {@code exposeHelpers} methods to add custom
	 * helpers.
	 * 
	 * @param velocityContext Velocity context that will be passed to the template
	 * @param request         current HTTP request
	 ...
	 * <p>
	 * Can be overridden to check for special callback interfaces, for example the
	 * ViewContext interface which is part of the view package of Velocity Tools. In
	 * the particular case of ViewContext, you'll usually also need a special
	 ...
	 * {@code mergeTemplate} method to merge the template instance with the given
	 * Velocity context.
	 * <p>
	 ...
	 * <p>
	 * Can be called by subclasses to retrieve a specific template, for example to
	 * render multiple templates into a single view.
	 * 
	...
	/**
	 * Subclass of DateTool from Velocity Tools, using a Spring-resolved Locale and
	 * TimeZone instead of the default Locale.
	 * 
	 * @see org.springframework.web.servlet.support.RequestContextUtils#getLocale
	...
	/**
	 * Subclass of NumberTool from Velocity Tools, using a Spring-resolved Locale
